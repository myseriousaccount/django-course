...
Механізми вище застосовні до будь-якої форми — і `forms.Form`, і `forms.ModelForm`. Нижче по два приклади на кожен тип, і в кожному підписано, який саме.

### 1. Публікація статті — `ModelForm`, успадковані правила
...
Тут працюють одразу дві різні речі: обмеження `title` (`max_length`) `ModelForm` бере з моделі сама — про це нічого не пишеш; а `clean_body()` — уже власне правило, якого модель не описує (мінімальний обсяг тексту).

> <i class="bi bi-info-circle"></i> `clean_slug()` тут насправді зайвий: `slug` у моделі — `SlugField(unique=True)`, а `ModelForm` сама перевіряє всі `unique=True`-поля під час `is_valid()`, коректно виключаючи поточний об'єкт при редагуванні. Лишений він для прикладу «як писати таку перевірку вручну» — і знадобиться, коли потрібне своє повідомлення про помилку або унікальність не описана в моделі напряму. Саме такий випадок — приклад 4 нижче: `email` у стандартного `User` **не** `unique=True`, тому там ручна перевірка обов'язкова.

### 2. Контактна форма — `forms.Form`, без моделі
...
### 3. Оформлення замовлення — `forms.Form`, верхня межа значення
...
### 4. Редагування профілю — `ModelForm`, унікальність без себе

На відміну від `slug` у прикладі 1, `email` стандартного `User` **не** `unique=True` — Django нічого не перевірить сама, ручна перевірка тут обов'язкова, а не про всяк випадок. Тонкий, але дуже частий нюанс: email має лишатися унікальним — але **не конфліктувати із самим користувачем**, якого ти зараз редагуєш. Тому в запиті виключаємо поточний обʼєкт через `.exclude(pk=...)`:
...
- `ModelForm` сама перевіряє `unique=True`-поля моделі (виключаючи поточний об'єкт); ручний `clean_<field>()` для унікальності пишуть лише там, де модель цього не описує.
