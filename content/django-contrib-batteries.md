...
> 🧠 Велика ідея: у Django **немає «ядра» й «надбудови»**. Адмінка, користувачі, сесії — не вшиті десь глибоко, а підключені як звичайні apps. Тому їх можна вмикати/вимикати рядком у `INSTALLED_APPS`. Хочеш сайт без сесій — прибрав рядок.
...
> 🧠 admin спирається на **auth** (треба залогінитись) і на **contenttypes** (щоб знати всі моделі). Батарейки працюють у зв'язці — саме тому кілька рядків у `INSTALLED_APPS` йдуть разом.
...
> ⚠️ **Прибрала рядок «щоб не заважав»** — батарейки залежні: приберешь `contenttypes` чи `auth`, і `admin` зламається. Не чіпай стандартний набір без потреби.

> ⚠️ **`messages` без відображення** — якщо додаєш `messages.success(...)`, але в базовому шаблоні немає циклу `{% for message in messages %}`, користувач нічого не побачить.

> ⚠️ **Пряме звертання до `User` через import** — краще `get_user_model()`, бо в проєкті модель користувача можуть замінити на власну. Це зробить код стійким до кастомного `User`.

> 💡 `staticfiles` у розробці роздає файли сам, а на продакшені треба `collectstatic` + окремий вебсервер/сховище. У навчальному проєкті про це можна поки не думати.
...
> 📖 Перелік вбудованих contrib-apps і їхні можливості — у доці: docs.djangoproject.com → «contrib packages».
