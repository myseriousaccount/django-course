...
> ⚠️ `.get()` кидає винятки, якщо кількість не дорівнює одиниці:
...
> 🧠 Аналогія з SQLAlchemy: `filter()` тут схожий на `Query.filter_by()`, а `.get()` — на `session.get()`, тільки Django кидає власні винятки моделі замість повернення `None`.
...
> 💡 Хочеш перевірити наявність — пиши `if qs.exists():`, а не `if qs:`. Обидва працюють, але `.exists()` виконує легкий запит замість завантаження всього набору.
...
> ⚠️ Зворотний бік — кешування. Якщо звернутися до одного QuerySet двічі в різних місцях, він може виконати запит повторно. Коли результат потрібен кілька разів, збережи його в список: `movies = list(qs)`.
...
> ⚠️ `bulk_create()` **не викликає** метод `save()` моделі й сигнали `pre_save`/`post_save` — якщо в моделі є логіка в `save()`, масове створення її омине.
...
> ⚠️ `queryset.update()`, як і `bulk_create`, НЕ викликає `save()` моделі й сигнали. Якщо в моделі є кастомна логіка в `save()`, масовий `update()` її омине — враховуй це.
...
> ⚠️ Видалення каскадне: якщо в об'єкта є пов'язані записи через `ForeignKey(on_delete=CASCADE)`, вони теж зникнуть. Наприклад, видалення `Post` забере й усі його коментарі.
...
> ⚠️ **`.get()` без гарантії унікальності** → `MultipleObjectsReturned`. Використовуй `.get()` лише за унікальним полем; для «одного зі списку» бери `.filter(...).first()`.

> ⚠️ **`if qs:` замість `if qs.exists():`** — завантажує весь набір заради простої перевірки. Для «чи є хоч один» — `.exists()`.

> ⚠️ **`update()` / `bulk_create()` минають `save()` і сигнали** — кастомна логіка моделі не спрацює. Коли вона потрібна, ітеруй і викликай `.save()`.

> 💡 Той самий результат кілька разів? Збережи `list(qs)`, щоб не повторювати SQL.
...
> 📖 Повний довідник — розділи «Making queries» та «QuerySet API reference» в офіційній документації (docs.djangoproject.com).
