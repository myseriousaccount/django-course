...
Ти один раз описуєш правильну поведінку («сторінка списку постів має повертати код 200 і містити заголовок»), а потім будь-коли запускаєш усі тести однією командою. Django сам проганяє їх і каже, що зламалося.

Головна цінність — **зміни не ламають наявне непомітно**. Ти дописала нову фічу, запустила тести, і одразу бачиш: стара сторінка кошика перестала працювати. Без тестів ти дізналася б про це від користувача через тиждень. Тести — це страхувальна сітка, яка дозволяє сміливо міняти код.
...
Перед запуском тестів Django створює **тимчасову** БД, а після — видаляє її. Кожен окремий тест виконується в **транзакції**, яка **відкочується** після завершення. Тобто дані, які ти створила в одному тесті, зникають перед наступним — тести не впливають один на одного.

Тестова база **окрема від реальної**. Хоч скільки об'єктів ти створюй у тестах, твоя справжня БД із реальними постами й замовленнями лишається недоторканою.
...
Django шукає тести у файлі `app/tests.py`, який створюється разом із кожним застосунком. Коли тестів стає багато, файл замінюють на **пакет** — папку `tests/` із `__init__.py` та кількома файлами (`test_models.py`, `test_views.py`, `test_forms.py`).
...
Правило пошуку одне: файли мають починатися з `test`, класи успадковувати `TestCase`, а методи-тести — починатися з `test_`. Тільки такі методи Django запускає як тести.
...
`setUpTestData` **швидший** за `setUp`: дані створюються раз, а не перед кожним методом. Бери його для об'єктів, які тести лише читають. `setUp` потрібен, коли тест **змінює** об'єкт і кожному потрібна свіжа копія.
...
Ти кажеш клієнту «зайди на цю адресу» — він проганяє повний цикл (URLconf → view → шаблон) і віддає тобі `response`. Далі перевіряєш поля відповіді.
...
Три шари, які перевіряють окремо: обчислення в моделі, правила форми й відповідь сторінки.
...
> <i class="bi bi-info-circle"></i> Не тестуй сам Django — те, що `create()` збереже об'єкт, вже перевірено розробниками фреймворку. Тестуй **свій** код: методи, які написала ти.
...
Django сам знаходить усі тести й запускає їх однією командою:
...
|---|---|
| Метод названо не з `test_` | Django його не запускає, і тест мовчки не виконується |
| Дані створюються в кожному тесті окремо | Повільно; спільні об'єкти виносять у `setUpTestData` |
| Тест покладається на порядок виконання | Кожен тест виконується в окремій транзакції; залежність між тестами робить набір нестабільним |
| Тестується поведінка самого Django | `create()` і `filter()` уже перевірені у фреймворку; тестують власні методи, форми й views |
| Адреси в тестах написані рядком | Після зміни маршруту тест перестає перевіряти те, що треба; використовують `reverse()` |
| Тест звертається до реальної бази проєкту | `TestCase` створює окрему тестову базу й відкочує зміни; окремих налаштувань для цього не потрібно |
| Зовнішні сервіси викликаються по-справжньому | Тести стають повільними й нестабільними; такі виклики підміняють через `unittest.mock` |
