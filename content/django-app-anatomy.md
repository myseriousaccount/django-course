...
Кожен файл застосунку відповідає за свій шар: опис даних, обробку запиту, адреси, форми, адмінку й налаштування самого модуля.
...
Головна таблиця не описує адреси застосунку сама, а лише підключає його `urls.py` через `include()`. Побудова адрес, іменовані маршрути й простір імен для кількох застосунків — уроки «URL-маршрути» та «Простори імен маршрутів».
...
- `urls.py` застосунку підключається в головній таблиці через `include()`; побудова адрес і простір імен — окремі уроки.
