...
> 🧠 Якщо ти працювала з Flask — це прямий аналог `render_template('page.html', **context)`. Різниця лише в тому, що Django вимагає передати `request` явним першим аргументом (принцип «явність краще за неявність»).
...
> 💡 Щоб працював `redirect(obj)`, у моделі має бути метод `get_absolute_url()`. Він повертає рядок-шлях до сторінки об'єкта — Django викличе його автоматично.
...
> 💡 Пам'ятай про Post/Redirect/Get: після успішного POST (створили замовлення, додали коментар) завжди роби `redirect`, а не `render`. Інакше при оновленні сторінки браузер повторно надішле форму.
...
> ⚠️ Перший аргумент — це **сама модель або QuerySet**, а не рядок. `get_object_or_404(Movie, pk=pk)`, а не `get_object_or_404('Movie', ...)`.

> ⚠️ Якщо умова знайде **більше одного** об'єкта, `get_object_or_404` кине `MultipleObjectsReturned` (це вже помилка 500, не 404). Фільтруй так, щоб результат був однозначний — за унікальним полем (`pk`, `slug`).
...
> 💡 Використовуй його лише тоді, коли порожній результат — це справді «нічого не знайдено» (помилка адреси). Якщо порожній список — нормальний стан («у цього автора поки немає книг»), бери звичайний `.filter()` і показуй відповідне повідомлення в шаблоні.
...
> ⚠️ **Забутий `return` перед `render`/`redirect`** → `The view didn't return an HttpResponse object`. Ці хелпери лише **створюють** відповідь — повернути її мусиш ти.

> ⚠️ **Рядок замість моделі** у `get_object_or_404('Product', ...)` — перший аргумент завжди сам клас моделі або QuerySet.

> ⚠️ **`get_list_or_404` там, де порожнеча нормальна** — користувач отримає 404 на цілком робочій сторінці. Для «поки нічого немає» бери `.filter()`.

> 💡 Після POST — завжди `redirect`, а не `render` (Post/Redirect/Get), щоб оновлення сторінки не відправило форму повторно.
...
> 📖 Повний перелік хелперів — у розділі «django.shortcuts» офіційної документації (docs.djangoproject.com).
