...
❓ Який `detail` мається на увазі — поста чи товару? Django не вгадає однозначно. Імена маршрутів **глобальні** на весь проєкт, тож два однакові `name='detail'` конфліктують: Django візьме той, що зареєстрований останнім, і посилання «поламаються» непередбачувано.
...
> 🧠 Аналогія: `app_name` — це як **прізвище**. У класі може бути дві Олени, тому кличуть «Олена Коваль» і «Олена Шевченко». `blog:detail` і `shop:detail` — той самий принцип: ім'я + «прізвище» модуля.
...
> ⚠️ Кількість і порядок аргументів у `{% url %}` мають збігатися з динамічними частинами `path()`. Для `path('<int:pk>/reviews/<slug:slug>/')` треба передати **обидва** значення.
...
> 💡 Хороша звичка: додавати `app_name` у **кожен** `app/urls.py` одразу, ще до того, як виникли конфлікти. Це дешево зараз і рятує від плутанини потім. Тоді ти завжди пишеш `blog:index`, `shop:index` — послідовно, без винятків.
...
> ⚠️ **`NoReverseMatch`** — Django не знайшов маршрут за цим іменем. Причини: забула `app_name`, друкарська помилка у префіксі (`blg:detail`), або передала не ту кількість аргументів.

> ⚠️ **Забула префікс після додавання `app_name`** — щойно з'являється `app_name`, старе `{% url 'detail' %}` перестає працювати; треба всюди `{% url 'blog:detail' %}`. Це очікувано: namespace тепер обов'язковий для цього app.

> ⚠️ **Плутанина `args` vs `kwargs`** у `reverse` — `args=[...]` для позиційних, `kwargs={...}` для іменованих частин `path()`. Змішувати в одному виклику не можна.
...
> 📖 Деталі — у доці: docs.djangoproject.com → «URL dispatcher» → «URL namespaces» і «Reversing namespaced URLs».
