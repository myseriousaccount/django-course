...
Якщо скрипт виконається раніше, ніж браузер побудує HTML, `$('#cart')` просто нічого не знайде — і код мовчки не спрацює.
...
Чотири базові дії jQuery на сторінці: знайти елемент, прочитати або змінити його, дійти від нього до сусідів і створити новий.
...
```js
// static/js/app.js — приклади селекторів
$('#cart-qty')                   // за id
...
// прибрати картку фільму зі списку "до перегляду"
...
// додати учня в список без перезавантаження
...
```js
// static/js/library.js — гілка обробки помилки
error: function (xhr) {
...
Дані з бази йдуть у `data-`атрибути:
...
|---|---|
| Обробник навішано на елементи, яких ще немає | Кнопки, домальовані запитом, не реагують; потрібне делегування `.on('click', '.selector', …)` |
| POST без CSRF-токена | 403 при першому ж запиті |
| Написано лише `success`, без `error` | При 403 чи 500 користувач бачить, що «нічого не сталося» |
| Змішування jQuery-обгортки й DOM-елемента | `$('#x').value` дає `undefined`; для jQuery це `.val()` |
| Немає захисту від подвійного кліку | За час запиту користувач устигає створити два записи; кнопку вимикають на час виконання |
| Порожній селектор | Помилки не буде, код просто нічого не зробить; перевіряють через `.length` |
| Плагін підключено вище за jQuery | `$ is not defined`: спершу бібліотека, потім плагіни, потім власний код |
