...
> <i class="bi bi-info-circle"></i> Django дозволяє замінити `User` на власну модель (`AUTH_USER_MODEL`), якщо потрібні свої поля. Для навчальних проєктів достатньо стандартної. Але звертайся до неї через `get_user_model()`, якщо пишеш перевикористовуваний код. Якщо своя модель уже є — адмінку для неї (`UserAdmin`, `fieldsets` і `add_fieldsets`) розбирає урок «Адмін-панель».

## Створення користувача: `create_user()`
...
Два кроки, які часто плутають: спершу Django перевіряє пару логін-пароль, і лише потім записує користувача в сесію.
...
Звідки будь-яка сторінка знає поточного користувача — і чому для цього не треба нічого імпортувати у view.
...
Одне правило «лише для залогінених» у трьох варіантах: для функції, для класу й у налаштуваннях проєкту.

### @login_required — для функцій
