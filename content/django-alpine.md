...
> 🧠 Аналогія: Alpine — це як теги `{% if %}` / `{% for %}` у шаблоні, але виконуються **в браузері** й **реагують на дії** користувача. Django-теги вирішують, який HTML віддати; Alpine-атрибути вирішують, як він поводиться після відкриття.
...
> ⚠️ Атрибут `defer` **обов'язковий** — щоб Alpine запустився після побудови HTML.
...
> 💡 Саме так зроблені вкладки на цій платформі: у коді `index.html` є `x-data="app()"` зі станом `activeTab`, а кнопки роблять `@click="activeTab = 'theory'"` — той самий патерн, тільки стан винесено в окрему функцію.
...
> 💡 Приємний бонус: Alpine керує текстом через `x-text`, а **не** через `{{ }}`, тож він **не конфліктує** з шаблонними тегами Django `{{ }}`. Обидва спокійно живуть в одному файлі.
...
> ⚠️ Забути `defer` у `<script>` Alpine — тоді він спробує запуститись раніше за HTML і нічого не «оживить».

> ⚠️ `x-show` лишає елемент у DOM (лише ховає через CSS), а `x-if` **прибирає** його зовсім. `x-if` і `x-for` працюють **тільки** всередині `<template>`.

> 💡 Alpine — про зручність у браузері, а не про безпеку. Валідацію й перевірку прав **завжди** роби на сервері (Django), бо будь-який Alpine-код відкритий у браузері.
...
> 📖 Офіційна документація — alpinejs.dev (розділ «Directives»); а живий приклад завжди під рукою — код `index.html` цієї платформи.
