...
<i class="bi bi-lightbulb"></i> Логіка назв проста: `db` — усе про базу, `http` — усе про запит і відповідь, `contrib` — готові підсистеми, `core` — внутрішня механіка, доступна й тобі, `utils` — дрібні помічники. Якщо не знаєш, де шукати, спитай себе, до якої з цих п'яти категорій належить задача.
...
Що саме дає кожна батарейка — в уроці «Вбудовані застосунки».
...
**Навіщо саме так, а не `import settings`.** `django.conf.settings` враховує і твій `settings.py`, і значення за замовчуванням, і змінні оточення — це єдина правильна точка доступу.
...
> <i class="bi bi-exclamation-triangle"></i> **`models.CharField` проти `forms.CharField`.** Різні речі з однаковими іменами. Помилка «поле не зберігається в базі» часто саме звідси.

> <i class="bi bi-exclamation-triangle"></i> **Імпорт `User` заради поточного користувача.** `from django.contrib.auth.models import User` потрібен для **запитів до таблиці користувачів**. Поточний користувач приходить сам — у `request.user`, без жодного імпорту.

> <i class="bi bi-info-circle"></i> **`get_user_model()` замість прямого `User`.** У великих проєктах модель користувача часто замінюють власною. Тоді `from django.contrib.auth import get_user_model` і `User = get_user_model()` — код не зламається після заміни. Для навчального проєкту прямий імпорт цілком нормальний.

> <i class="bi bi-info-circle"></i> **Циклічні імпорти між apps.** Якщо `orders/models.py` імпортує з `catalog/models.py`, а той назад — Python впаде. Рішення: посилатися рядком, `models.ForeignKey('catalog.Product', ...)`.

> <i class="bi bi-info-circle"></i> **Зовнішні бібліотеки живуть окремо.** `rest_framework`, `crispy_forms`, `pillow` — це не `django.*`. Їх спершу ставлять через `pip`, часто додають в `INSTALLED_APPS`, і лише потім імпортують.
