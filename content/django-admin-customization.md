...
> 💡 Щоб об'єкти показувалися людською назвою, а не `Post object (1)`, у **моделі** додай `__str__`:
> ```python
> def __str__(self):
>     return self.title
> ```
...
> 💡 Обидва способи працюють однаково — результат ідентичний. Декоратор `@admin.register` лаконічніший і тримає налаштування та реєстрацію в одному місці, тож у новому коді обирай саме його.
...
Це сторінка з переліком усіх об'єктів. Уяви `ModelAdmin` як пульт керування 🧠: кожна опція вмикає окрему зручність. Розберемо на моделі книги в каталозі:
...
> ⚠️ Щоб `autocomplete_fields = ("director",)` працювало, у `ModelAdmin` **зв'язаної** моделі (`DirectorAdmin`) мають бути задані `search_fields` — саме по них іде пошук.
...
> 💡 Значення дописується лише **під час створення** об'єкта — якщо потім змінити `title`, наявний `slug` не перезапишеться. Це навмисно: slug часто вже потрапив у посилання, і мовчазна його зміна ламала б URL.
...
> ⚠️ **Модель не видно в адмінці.** Найчастіша причина — її просто не зареєстрували в `admin.py`. Немає реєстрації — немає моделі в панелі, помилки при цьому теж немає.

> ⚠️ **Поле у `list_editable` має бути й у `list_display`.** Редагувати у списку можна лише те, що в цьому списку показано. Інакше Django видасть помилку під час запуску.

> ⚠️ **Перше поле `list_display` не можна класти в `list_editable`.** Воно працює як посилання на сторінку об'єкта (`list_display_links`), тож редагованим бути не може.

> ⚠️ **`autocomplete_fields` без `search_fields`.** Автодоповнення шукає по `search_fields` зв'язаної моделі — якщо їх немає, буде помилка перевірки.

> 💡 `search_fields` шукає лише за **текстовими** полями (`CharField`, `TextField`) — можна й через `__` по зв'язаних (`author__name`). Число чи булеве поле туди не клади — для них є `list_filter`.
...
> 📖 Повний перелік опцій `ModelAdmin`, inline-класів і можливостей кастомізації — у розділі «The Django admin site» документації (docs.djangoproject.com).
