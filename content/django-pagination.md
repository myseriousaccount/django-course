...
`Paginator` ділить довгий список на сторінки заданого розміру й рахує все потрібне для навігації: кількість сторінок, наявність сусідніх, діапазон показаних елементів. Урок про використання у view і в шаблоні.
...
Ти передаєш йому весь список і кількість елементів на сторінку — а він рахує, скільки вийде сторінок і що на кожній.
...
Замість того щоб самому рахувати `OFFSET`/`LIMIT` і межі сторінок, ти делегуєш це фреймворку. Менше коду — менше помилок «на межі» (типу пропущеного чи задвоєного елемента).
...
Номер сторінки приходить із запиту як GET-параметр `?page=2`. Метод `get_page()` бере цей номер і повертає відповідну сторінку.
...
`get_page()` — безпечний метод: він **не падає** на некоректному вводі. Якщо `page` порожній чи не число — повертає першу сторінку; якщо номер завеликий — останню. Тобі не треба ловити винятки вручну.
...
Об'єкт сторінки (`page`), який повертає `get_page()`, — це не просто список: він знає свій номер, чи є сусідні сторінки й скільки сторінок усього.
...
З цих атрибутів будується блок навігації: кнопки «‹ Назад» / «Далі ›», напис «сторінка 2 з 7» і навіть «показано 11–20 із 143» (через `start_index`/`end_index`). Усе потрібне вже пораховано.
...
Код пагінації не залежить від предметної області: змінюються лише QuerySet, розмір сторінки й шаблон. Для стрічки блогу це був би `Post.objects.filter(is_published=True).order_by('-created')` і `Paginator(posts, 5)`.
...
## Коли пагінація потрібна

Списки, що ростуть із часом — статті, товари, книги, замовлення — пагінують завжди: без цього сторінка з часом починає віддавати тисячі рядків і сотні зображень. Короткі й обмежені за розміром списки (категорії, пункти меню, статуси) не пагінують — це зайве ускладнення без виграшу.

Розмір сторінки обирають за вагою елемента: 5–10 для довгих карток зі зображеннями, 20–50 для табличних рядків.

## Типові помилки / Нюанси

| Що не так | Наслідок і як правильно |
|---|---|
| `next_page_number` без перевірки | `EmptyPage` на останній сторінці; звертання огортають у `{% if page.has_next %}` |
| Немає `order_by` перед пагінацією | Порядок рядків не гарантований: об'єкт може повторитися на двох сторінках або зникнути |
| Посилання `?page=2` без інших параметрів | Затирає активний пошук і фільтри; параметри переносять вручну або власним тегом |
| `paginator.page(number)` замість `get_page()` | Некоректний `?page=abc` дає виняток; `get_page()` повертає першу або останню сторінку |
| Пагінація списку, завантаженого в пам'ять | `Paginator(list(qs), 20)` тягне всі рядки з бази; передавай сам QuerySet — тоді Django обмежить вибірку на рівні SQL |
| `count` у циклі шаблону | Кожен виклик — окремий запит; загальну кількість беруть з `page.paginator.count` |
